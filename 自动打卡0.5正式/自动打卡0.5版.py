import requests
import json
from sys import exit#控制程序手动退出
import smtplib#用来连接登录邮箱 
from email.mime.text import MIMEText#用来创建邮件对象
from email.header import Header#用来构造邮件主题
from datetime import date#用来获取当日日期
info=(json.loads(open("info.json",'r',encoding="utf-8").read()))#载入配置文件，包含账号 密码 JWSESSION location
header={"Host": "gw.wozaixiaoyuan.com",#此header为小黄鸟抓包得到的http传输中的header 作用是伪装模拟
            "Connection": "keep-alive",
            "Accept": "application/json, text/plain, */*",
            "JWSESSION": info['JWSESSION'],#从配置中读取
            ...
            #"Referer": "https://gw.wozaixiaoyuan.com/health/mobile/health/getBatch",这个参数需要在不同函数中有所变化
...
def GetState():#这个函数调用来测试当前储存的JWSESSION是否过期
    ...
    header['JWSESSION']=info['JWSESSION']
    body={}
...
def login():
    ...
    header['Content-Length']="2"
    header['Referer']='https://gw.wozaixiaoyuan.com/h5/mobile/basicinfo/index/login/index?jwcode=10'
    ...
    if(json.loads(response.text)["code"]==0):#code为0则登陆成功 此处调用json库将数据解码为python字典格式，更方便这里的判断语句
        info['JWSESSION']=response.headers['JWSESSION']#更新JWSESSION到字典中
        header['JWSESSION']=info['JWSESSION']#写入header字典
        file=open('info.json','w',encoding='utf-8')#打开文件，使用覆盖写模式
        file.write(json.dumps(info,ensure_ascii=False)) #将获取到的JWSESSION参数覆盖写入文件
        file.close()#关闭文件
        return True
    elif(json.loads(response.text)['code']==101):#101表示账号密码错误
        SendMail(info['receiver'],'账号或密码错误')
        print ("用户名或密码错误，请检查")
    else:
        print("未知错误")
    exit(0)#登陆失败 直接退出程序 反正后面也没办法进行打卡了
...
def GetForm():#此函数作用是获取打卡的选项并选择答案
    ...
    res={"location":info['location']}#初始化一个res字典 马上派上用场
        ...
        for number in options:
            for objects in options[number]:
                if objects['type']==0 or objects['type']==2:#type为0则在客户端对应的是绿色的答案，优先选择。为2则是黑色答案 随便选 这里默认选第一个
                    ...
                    break
        answers={"location":info['location'],
...
def SendMail(receiver,state):#发送邮件提醒结果 需要两个参数：收件邮箱和打卡结果(成功|失败)
    server=smtplib.SMTP_SSL("smtp.qq.com",465)
    mes=MIMEText("感谢使用打卡助手",'plain','utf-8')
    mes['From']="SWPU健康打卡助手"
    mes['To']=f'{receiver}'#邮件收件人
    mes['Subject']=Header(f'{date.today()}打卡{state}')#邮件主题
    server.login("WZXY_Helper@qq.com","yeonkdwgledveaeb")
    server.sendmail("WZXY_Helper@qq.com",receiver,mes.as_string())
    server.quit()
...
def main():
    ...
    if(json.loads(r.text)["code"]==0):#为0则打卡成功
            #print("OK!")#反馈正确
            SendMail(info['receiver'],"成功")
            exit(0)
    elif(json.loads(r.text)["code"]==1):#为1表示打卡时间未开始
            print("打卡时间未开始，打卡失败")
    else:
        print("未知错误")
    SendMail(info['receiver'],"失败")
    exit()
